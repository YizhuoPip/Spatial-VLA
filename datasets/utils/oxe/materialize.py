...
def make_oxe_dataset_kwargs(
    ...
    """
    Generates config (kwargs) for given dataset from Open-X Embodiment. Single

    return: 
    dict["image_obs_keys"],["depth_obs_keys"],["state_obs_keys"],["absolute_action_mask"],["action_normalization_mask"]
...
def get_oxe_dataset_kwargs_and_weights(
    data_root_dir: Path,
    mixture_spec: List[Tuple[str, float]], #包含的数据集名称和采样权重
    load_camera_views: Tuple[str] = ("primary",),
    load_depth: bool = False,
    load_proprio: bool = True,
    load_language: bool = True,
    ...
    (per-dataset configs) and weights can be passed directly to `make_interleaved_dataset`. Mix
