...
configuration and initialization for the RLDS dataset
"""
...
# 确保PyTorch能够独占GPU进行模型训练，而TensorFlow的数据预处理操作只在CPU上运行
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    :param name: The name of the RLDS dataset.
    :param data_dir: The path to the data directory.
    :param train: Whether to use the training or validation split.
    :param shuffle: Whether to shuffle the file read order (does NOT fully shuffle the dataset, since one file usually contains many trajectories)!
    :param standardize_fn: A function that, if provided, will be the first thing applied to each trajectory.
    :param image_obs_keys (Mapping[str, str|None]): Mapping from {new: old} indicating which RGB images to extract from the
    ...
    :param depth_obs_keys (Mapping[str, str|None]): Same as `image_obs_keys`, but for depth images. Keys will be
        prefixed with "depth_" instead of "image_".
    :param state_obs_keys (Sequence[str|None]): List of 1-dimensional proprioception keys to be extracted from the
        "observation" dict, concatenated, and mapped to "proprio". Inserts 1 element of padding for each None entry.
    :param language_key (str, optional): If provided, the "task" dict will contain the key "language_instruction",
        extracted from `traj[language_key]`. key就是在这个dataset中traj中的关键词。
    :param action_proprio_normalization_type (str, optional): The type of normalization to perform on the action,
        proprio, or both. Can be "normal" (mean 0, std 1) or "bounds" (normalized to [-1, 1]).
    :param dataset_statistics: (dict|str, optional): dict (or path to JSON file) that contains dataset statistics
    ...
    :param absolute_action_mask: A sequence of booleans indicating which action dimensions are absolute.(纬度是某一个时间步的维度)
    :param action_normalization_mask (Sequence[bool], optional): If provided, indicates which action dimensions
    ...
    # 为什么traj map了两次，因为dataset_statistics中间修改了。
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    Args:
        ...
        future_action_window_size (int, optional): The number of future actions beyond window_size to include in the chunked actions.
        subsample_length (int, optional): If provided, trajectories longer than this will be subsampled to this length (after goal relabeling and chunking).
...
def make_interleaved_dataset(
    ...
    *,
    ...
    # sample weight 由dataset_size 决定
    ...
    # Interleave at the Frame Level 帧级别
